...
def home(request):
    ...
    context = {'shops':shops,'category':sorted(map(lambda x:x[1],CATEGORY_BUSINESS)),'categorIndex':map(lambda x:x[0],CATEGORY_BUSINESS),'massage':massage}
    return render(request, 'index.html', context) #sorted business categories
...
def searchByCategory(request,categ):
    ...
    context = {'shops':shops,'categ':q,'categName':categ}
    return render(request,'serchByCategory.html', context)
